...
def create_bank():
    ...
    # Prevent duplicate bank record for the same employee_id
    existing = Bank.query.filter_by(employee_id=data['employee_id']).first()
    if existing:
        return jsonify({'error': 'A bank record for this employee already exists.'}), 400
    record = Bank(
